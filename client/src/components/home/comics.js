...
import LikeButton from './likeButton'
 ...
 class Comics extends Component {
  ...
  componentDidMount(){
    // $(document).foundation();
     this.props.fetch({category : 'comics'})
    //  $(document).foundation();
...
$(function() {
  ...
  changeStatus(event){
     ...
     componentWillUnmount(){
        this.props.clear({category : 'comics', data : []})
    }      
  render() {
      return (
        ...
        <div className="col-12 col-md-12">
            <Carousel indicators={false}>
                {
                    this.props.comics.length > 0 &&  this.props.comics.map((project,i)=>{
                    return(
                    <Carousel.Item key={i}>   
...
function mapStateToProps(state){
 return {
     comics : state.projectInfo.comics
 }
}
