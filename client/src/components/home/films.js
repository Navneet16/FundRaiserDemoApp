...
import LikeButton from './likeButton'
 ...
 class Films extends Component {
  ...
  componentDidMount(){
    // $(document).foundation();
     this.props.fetch({category : 'films'})
    //  $(document).foundation();
...
$(function() {
  ...
  changeStatus(event){
     ...
     componentWillUnmount(){
        this.props.clear({category : 'films', data : []})
    }   
  render() {
      return (
        ...
        <div className="col-12 col-md-12">
            <Carousel indicators={false}>
                {
                    this.props.films.length > 0 &&  this.props.films.map((project,i)=>{
                    return(
                    <Carousel.Item key={i}>   
