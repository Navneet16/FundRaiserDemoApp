...
import LikeButton from './likeButton'
 ...
 class Technology extends Component {
  ...
  componentDidMount(){
    // $(document).foundation();
     this.props.fetch({category : 'technology'})
    //  $(document).foundation();
...
$(function() {
  ...
  componentWillUnmount(){
        this.props.clear({category : 'technology', data : []}) 
  }   
  render() {
      return (
        ...
        <div className="col-12 col-md-12">
            <Carousel indicators={false}>
                {
                    this.props.technology.length > 0 &&  this.props.technology.map((project,i)=>{
                    return(
                    <Carousel.Item key={i}>   
...
function mapStateToProps(state){
 return {
     technology : state.projectInfo.technology
 }
}
