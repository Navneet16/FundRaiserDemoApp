...
import LikeButton from './likeButton'
 ...
 class Music extends Component {
  ...
  componentDidMount(){
    // $(document).foundation();
     this.props.fetch({category : 'music'})
    //  $(document).foundation();
...
$(function() {
  ...
  changeStatus(event){
     ...
     componentWillUnmount(){
        this.props.clear({category : 'music', data : []})
    }      
  render() {
      return (
        ...
        <div className="col-12 col-md-12">
            <Carousel indicators={false}>
                {
                    this.props.music.length > 0 &&  this.props.music.map((project,i)=>{
                    return(
                    <Carousel.Item key={i}>   
